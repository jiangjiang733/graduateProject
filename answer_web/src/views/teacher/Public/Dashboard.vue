...
const formatSmartTime = (timeStr) => {
  ...
  // 超过48小时但在同一年：显示"月日"（如"1月8日"）
  if (date.getFullYear() === now.getFullYear()) {
    return `${date.getMonth() + 1}月${date.getDate()}日`
  }
  
  // 超过一年：显示"年月日"（如"2025年1月8日"）
...
.timeline-avatar-real :deep(.el-avatar) {
  border-radius: 16px;
}
