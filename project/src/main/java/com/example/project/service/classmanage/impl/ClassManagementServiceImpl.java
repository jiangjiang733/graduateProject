...
public class ClassManagementServiceImpl implements ClassManagementService {
    ...
    public StudentProgressDTO getStudentProgress(String studentId, String courseId) {
        ...
        int gradedReports = 0;
        BigDecimal totalReportScore = BigDecimal.ZERO;

        for (StudentLabReport report : studentReports) {
            if (report.getStatus() >= 1) { // 已提交
                submittedReports++;
            }
            if (report.getStatus() == 2 && report.getScore() != null) { // 已批改
                gradedReports++;
                totalReportScore = totalReportScore.add(report.getScore());
            }
        }

        progressDTO.setSubmittedReports(submittedReports);
        progressDTO.setGradedReports(gradedReports);

        if (gradedReports > 0) {
            BigDecimal averageScore = totalReportScore.divide(new BigDecimal(gradedReports), 2, RoundingMode.HALF_UP);
            progressDTO.setAverageReportScore(averageScore);
        } else {
            progressDTO.setAverageReportScore(BigDecimal.ZERO);
        }
