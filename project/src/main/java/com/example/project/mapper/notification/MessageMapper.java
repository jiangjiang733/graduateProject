...
public interface MessageMapper extends BaseMapper<Message> {
        ...
        @Select("SELECT m.*, " +
                        ...
                        "LEFT JOIN course_enrollment ce ON m.message_type = 'COURSE_INVITATION' AND m.related_id = CAST(ce.id AS CHAR) "
                        +
        ...
        /**
         * 根据 related_id 批量更新消息内容（用于评论删除时保留通知但遮蔽内容）
         */
        @Update("<script>" +
                        "UPDATE message SET content = #{content} WHERE related_id IN " +
                        "<foreach collection='relatedIds' item='id' open='(' separator=',' close=')'>" +
                        "#{id}" +
                        "</foreach>" +
                        "</script>")
        int updateContentByRelatedIds(@Param("relatedIds") List<String> relatedIds, @Param("content") String content);
}