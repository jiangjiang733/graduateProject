...
public interface StudentCourseMapper extends BaseMapper<StudentCourse> {
         ...
         */
        @Select({
                        "SELECT sc.*, s.students_username as studentName, s.students_head as studentAvatar",
                        "FROM student_course sc",
                        "INNER JOIN student_user s ON sc.student_id = s.students_id",
                        "INNER JOIN course c ON sc.course_id = c.id",
                        "WHERE sc.course_id = #{courseId}",
        ...
        @Select({
                        "<script>",
                        "SELECT sc.*, s.students_username as studentName, s.students_head as studentAvatar, c.name as courseName",
                        "FROM student_course sc",
                        "INNER JOIN student_user s ON sc.student_id = s.students_id",
                        "INNER JOIN course c ON sc.course_id = c.id",
                        "INNER JOIN teacher_user t ON c.teacher_id = t.teacher_id",
